...
void mixData(TString input = "Run2012", int cutNj = 4, int cutPt = 35, int cutNb = 2, int cutQ = 0)
{
  cout << endl << " ************************************************* " << endl
  ...
  TFile* file1 = TFile::Open(Form("../ntuples_oldPUweight/result_%s.root", sample));
  TFile* file2 = TFile::Open(Form("../ntuples_oldPUweight/result_%s.root", sample));
  ...
  TTree* tree1 = (TTree*)file1->Get("event/tree");//eventUp, eventDn,eventbJESUp,eventbJESDn
...
//    if ( abs(lepton1.eta()) < cut_maxMuonEta ) continue;
    ...
    for ( int j1=0, nj=event1.jets->size(); j1<nj; ++j1 )
    {
      ...
      double PUweight1 = 1;//event1.PUweight; //PUweight1 = 1;
      double Ptweight1 = 1;//event1.Ptweight;
...
*/
    ...
    for ( int l1=0; l1<13; ++l1 ) 
    {
      ...
      hnvertex->Fill(event1.vertex,muScale1);

      hSEvt_Mu_pt->Fill(lepton1.pt(),muScale1);
      hSEvt_Mu_eta->Fill(lepton1.eta(),muScale1);
      }
    }    

   //hSEvt_nLeadJets->Fill(nLeadJetsInSameEvt,event1.PUweight); nLeadJetsInSameEvt = 0;
    hSEvt_nJets->Fill(nJets1); hSEvt_nBJets->Fill(nBjets1);
    hSEvt_nLightJets->Fill(nLightJets1);
...
//      if ( abs(lepton2.eta()) < cut_maxMuonEta ) continue;
      else { hBEvt_nJets->Fill(nJets2); hBEvt_nBJets->Fill(nBjets2); }
    ...
    for ( int j1=0, nj1=event1.jets->size(); j1<nj1; ++j1 )
    {
      ...
      for ( int j2=0, nj2=event2.jets->size(); j2<nj2; ++j2 )
      {
        ...
        double dPhiJJ = ROOT::Math::VectorUtil::DeltaPhi(jet1,jet2);
        double dEtaJJ = jet1Eta-jet2Eta;
        double PUweight1 = 1;//event1.PUweight;
        double PUweight2 = 1;//event2.PUweight;
        double PUweight3 = PUweight1*PUweight2; //PUweight3 = 1;
        double Ptweight1 = 1;//event1.Ptweight;
        double Ptweight2 = 1;//event2.Ptweight;
